...
const ListingCard = ({ 
  ...
  return (
    <Card 
      className={`group cursor-pointer transition-smooth hover:shadow-medium bg-card border-border overflow-hidden ${variant === "list" ? "flex flex-row items-stretch min-h-[140px]" : ""} ${className}`}
      onClick={handleCardClick}
    >
      {/* Image Carousel */}
      <div 
        className={`relative ${variant === "list" ? "w-40 sm:w-48 flex-shrink-0" : ""}`}
      >
        {/* Paused Overlay */}
      ...
      {/* Content */}
      <div className={`${variant === "list" ? "p-3 sm:p-4 space-y-2 flex-1 min-w-0 flex flex-col" : "p-4 space-y-3"}`}>
        {/* Title and Price */}
        ...
        <div className={`flex items-center flex-wrap ${variant === "list" ? "gap-1.5" : "gap-2"}`}>
          ...
          {(typeof quantity === 'number' && quantity > 1) && (
            <Badge variant="outline" className={`bg-[#047857] text-white border-0 font-bold tracking-wider rounded-full shadow-sm flex items-center ${variant === "list" ? "gap-1 px-2.5 py-1 text-xs" : "gap-1.5 text-xs px-4 py-1.5"}`}>
              <Package className={variant === "list" ? "h-3 w-3" : "h-3.5 w-3.5"} />
              {quantity} UNITS
            </Badge>
          )}
          {carbonSaved > 0 && (
            <Badge variant="outline" className={`bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800 font-bold tracking-wider rounded-full shadow-sm flex items-center ${variant === "list" ? "gap-1 px-2.5 py-1 text-xs" : "gap-1.5 text-xs px-4 py-1.5"}`}>
              <Leaf className={variant === "list" ? "h-3 w-3" : "h-3.5 w-3.5"} />
              {carbonSaved.toFixed(0)} kg CO₂
            </Badge>
          )}
          {allowOffers && (
            <Badge variant="outline" className={`bg-[#8B5CF6] text-white border-0 font-bold tracking-wider rounded-full shadow-sm ${variant === "list" ? "px-2.5 py-1 text-xs" : "text-xs px-4 py-1.5"}`}>
              OPEN TO OFFERS
            </Badge>
          )}
        </div>
        ...
        <div className={`flex items-center justify-between text-muted-foreground ${variant === "list" ? "text-xs sm:text-sm" : "text-sm"}`}>
          <div className="flex items-center gap-1 min-w-0 flex-1">
            <MapPin className={variant === "list" ? "h-3.5 w-3.5 flex-shrink-0" : "h-3.5 w-3.5 flex-shrink-0"} />
            <span className="truncate">{location}</span>
          </div>
          <div className="flex items-center gap-1 flex-shrink-0">
            <Calendar className={variant === "list" ? "h-3.5 w-3.5" : "h-3.5 w-3.5"} />
        ...
        {(deliveryAvailable || pickupAvailable) && (
          <div className={`flex items-center gap-1 text-muted-foreground ${variant === "list" ? "text-xs sm:text-sm" : "text-sm pt-1"}`}>
            <Truck className={variant === "list" ? "h-3.5 w-3.5" : "h-3.5 w-3.5"} />
            <span>
              {deliveryAvailable && pickupAvailable 
                ? 'Pickup & Delivery'
                : deliveryAvailable 
        ...
        {(sellerVerified || sellerStripeVerified || sellerIdentityVerified) && (
          <div className={`${variant === "list" ? "pt-1.5 mt-auto border-t" : "pt-2 border-t"}`}>
            <VerificationBadges
